...
<?php $component->withAttributes([]); ?>
    ...
    <?php echo e(__('Mes consultations')); ?>
...
<a href="/patient/analyses" class="btn btn-outline-info ms-3 d-flex align-items-center" style="height:40px;">
                    ...
                    <div class="card-header bg-gradient-success text-white border-0 rounded-top">
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            <span class="badge bg-light text-dark">
                                <?php echo e(isset($appointments) ? $appointments->where('statut', '!=', 'refuse')->count() : 0); ?>

                            </span>
                        </div>
                    </div>
                    <div class="card-body p-0">
                        <?php if(isset($appointments) && $appointments->where('statut', '!=', 'refuse')->count()): ?>
                                ...
                                <table class="table table-hover mb-0 modern-table">
                                    ...
                                    <tbody>
                                        <?php $__currentLoopData = $appointments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $appointment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php if($appointment->statut !== 'refuse'): ?>
                                            <tr class="appointment-row">
                                                <td class="ps-4 py-3">
                                                    <?php $medecin = $appointment->medecin ?? \App\Models\User::find($appointment->medecin_id); ?>
                                                    <div class="d-flex align-items-center">
                                                        <div class="doctor-avatar me-3">
                                                            <i class="fas fa-user-md text-primary"></i>
                                                        </div>
                                                        <div>
                                                            <div class="fw-semibold text-dark"><?php echo e($medecin->name ?? 'N/A'); ?></div>
                                                            <small class="text-muted">
                                                                <i class="fas fa-stethoscope me-1"></i>
                                                                <?php echo e($medecin->specialite ?? 'Non définie'); ?>
                                                ...
                                                <td class="py-3">
                                                    ...
                                                    <small class="text-muted">
                                                        ...
                                                        <?php echo e($appointment->heure); ?>

                                                    </small>
                                                </td>
                                                <td class="py-3">
                                                    <?php if($appointment->type === 'en_ligne'): ?>
                                                ...
                                                <td class="py-3">
                                                    <?php if($appointment->statut === 'valide'): ?>
                                                ...
                                                <td class="py-3">
                                                    <div class="d-flex gap-2 flex-wrap">
                                                        <?php if($appointment->type === 'en_ligne' && $appointment->statut === 'valide'): ?>
                                                            <?php
                                                                $rdvDateTime = \Carbon\Carbon::parse($appointment->date . ' ' . $appointment->heure);
                                                                $now = \Carbon\Carbon::now();
                                                            ?>
                                                            <?php if($now->greaterThanOrEqualTo($rdvDateTime)): ?>
                                                                <button type="button"
   ...
   data-datetime="<?php echo e(\Carbon\Carbon::parse($appointment->date.' '.$appointment->heure)->format('Y-m-d\TH:i:s')); ?>">
    <i class="fas fa-play me-1"></i>
    Démarrer
</button>
                                                            <?php else: ?>
                                                                <button id="btn-demarrer-<?php echo e($appointment->id); ?>" type="button" disabled
...
<div id="countdown-<?php echo e($appointment->id); ?>" class="countdown-display small text-muted mt-1">
    <i class="fas fa-stopwatch me-1"></i>
</div>
                                                                <script>
                                                                    document.addEventListener('DOMContentLoaded', function() {
                                                                        function startCountdown_<?php echo e($appointment->id); ?>() {
                                                                            var rdvTime = new Date("<?php echo e($rdvDateTime ? $rdvDateTime->format('Y-m-d H:i:s') : ''); ?>").getTime();
                                                                            var x = setInterval(function() {
                                                                                var now = new Date().getTime();
                                                                                var distance = rdvTime - now;
                                                                                if (distance < 0) {
                                                                                    clearInterval(x);
                                                                                    document.getElementById('countdown-<?php echo e($appointment->id); ?>').innerHTML = '<span class="text-danger">Expiré</span>';
                                                                                } else {
                                                                                    var hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
                                                                                    var minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
                                                                                    var seconds = Math.floor((distance % (1000 * 60)) / 1000);
                                                                                    document.getElementById('countdown-<?php echo e($appointment->id); ?>').innerHTML = 
                                                                                        '<i class="fas fa-stopwatch me-1"></i>Dans ' + hours + 'h ' + minutes + 'm ' + seconds + 's';
                                                                                }
                                                                            }, 1000);
                                                                        }
                                                                        startCountdown_<?php echo e($appointment->id); ?>();
                                                                    });
                                                                </script>
                                                            <?php endif; ?>
                                                        <?php elseif($appointment->type === 'presentiel' && $appointment->statut === 'valide'): ?>
   ...
   data-medecin="<?php echo e($medecin->name ?? 'N/A'); ?>"
   data-specialite="<?php echo e($medecin->specialite ?? 'Non définie'); ?>"
   data-date="<?php echo e(\Carbon\Carbon::parse($appointment->date)->format('d/m/Y')); ?>"
   data-heure="<?php echo e($appointment->heure); ?>"
   data-type="<?php echo e($appointment->type === 'en_ligne' ? 'En ligne' : 'Présentiel'); ?>"
   data-statut="<?php echo e($appointment->statut); ?>">
    ...
    <div id="camera-popup" style="display:none; position:fixed; top:30px; right:30px; z-index:1050; background:#fff; border-radius:8px; box-shadow:0 2px 32px rgba(0,0,0,0.18); padding:0 0 12px 0; width:380px; max-width:98vw; min-width:260px;">
        <div style="display:flex; justify-content:space-between; align-items:center; background:#2196f3; border-radius:18px 18px 0 0; padding:13px 18px 13px 18px;">
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                <span style="font-weight:600; color:#fff; font-size:1.12rem;"><?php echo e(auth()->user()->name ?? 'Patient'); ?></span>
...
<?php endif; ?><?php /**PATH /home/tagne/Desktop/DSL/dermonline/resources/views/patient/consultations/index.blade.php ENDPATH**/ ?>